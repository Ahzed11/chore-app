...
| TASK-052, TASK-053 | ✅ Done 2026-07-16 — admin chore reassignment UI (member-picker sheet, pending/overdue only) and invite management (list active invites with expiry, revoke, generate only on explicit action, admin-gated section). |
...
| TASK-077, TASK-078, TASK-079 | ✅ Done 2026-07-15 — `POST /users/me/password` + `python -m app.cli reset-password` operator CLI; `DELETE /households/{id}` (confirm-gated hard delete) and `DELETE /users/me` (sole-admin guard, redistribution, token revocation); email lowercase normalization + migration + `lower(email)` unique index; leaderboard UTC + exclusive window bounds. |
| TASK-073, TASK-080 | ✅ Done 2026-07-16 — scheduler runs at startup + 6h misfire grace; backfill capped at `GRACE_DAYS` (default 3); one rotation lock per household per run; rotation-pointer modulo bug fixed with regression tests; invite-accept row lock; test suite 2:46 → ~1:10 via session-scoped schema + truncation. |
| TASK-074, TASK-075, TASK-076 | ✅ Done 2026-07-16 — `docker-compose.prod.yml` (GHCR image, env_file, migrations-on-start entrypoint, no reload/mount/debug); daily pg_dump backup sidecar + `make backup`/`make restore`; root README self-hosting guide (HTTPS proxy examples, backup/restore, APK notes). ⚠️ Not runtime-verified (no container runtime in the session) — first real deploy should confirm: image build with new entrypoint CMD, `prod-up` → migrate → healthy, backup sidecar output, full restore cycle. |
| TASK-081 | ⏳ Open — ntfy/Gotify notifications (`docs/backend-report.md` L10) |
