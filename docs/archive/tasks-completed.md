...
**Status 2026-08-18**: TASK-110's harness could NOT reproduce a layout failure
in the current sheet (Layer-1 sweep opens it with 15+ templates at phone sizes
and asserts zero exceptions; Layer-2 E2E exercises the full copy flow) — so the
task's planned hardening was applied anyway (below), per the canonical pattern,
and an adversarial review (fresh agent) drove a second round of fixes.

**Fix applied**:
- Sheet rebuilt as the canonical `DraggableScrollableSheet` pattern
  (`showModalBottomSheet(isScrollControlled: true, useSafeArea: true)` →
  `DraggableScrollableSheet(expand: false, initial: 0.5, min: 0.2, max: 0.85)`
  with a plain non-shrinkWrap `ListView` driven by its controller). The old
  `Flexible`-inside-`min`-Column + `shrinkWrap` ListView was fragile — with
  enough templates it could hit "RenderFlex children have non-zero flex but
  incoming height constraints are unbounded" or overflow past the screen.
- The header is the list's FIRST ITEM (scrolls with the rows) — a fixed
  header outside the scrollable would overflow at minChildSize on short
  landscape screens with large text (adversarial review finding).
- ListView padding restores the 16px side insets and adds the bottom
  safe-area inset (system nav bar / home indicator); `unfocus()` before
  opening so the sheet opens at full height.

**Tests (all required by the task body, added in this task)**:
- 15 templates: scroll to the LAST row, tap it, assert all 4 fields copied
  (create_chore_screen_test.dart).
- Templates provider error → `friendlyErrorMessage` snackbar (throwing
  notifier; previously untested).
- Sweep: 15-template variant asserts the sheet is a half-height panel, NOT
  full-screen (the old sheet's user-visible defect with many templates);
  LANDSCAPE + textScale 2.0 variant drags the sheet to minChildSize and
  back, asserting zero exceptions.

**Acceptance criteria**: `flutter analyze --no-pub --no-fatal-infos` clean;
`flutter test --no-pub` green (246 incl. the new tests); E2E journey green
(copy flow through the revised sheet); landscape variant deterministic (3/3).
...
`test_list_chores_pagination_is_stable` was updated, see Tests below)
...
**Changes**:
1. Backend — `backend/app/api/chores.py` `list_chores`: ORDER BY changed to
   `ChoreInstance.created_at.desc(), ChoreInstance.id.desc()` (id DESC keeps
   offset pagination deterministic on created_at ties; never order by
   created_at alone). Recurring instances generated by the scheduler sort to
   the top — the requested behaviour.
2. Backend — `ChoreInstanceResponse` gained `created_at: datetime`,
   populated in `_instance_response_from_row`; flows into
   create/complete/dismiss responses too.
3. Flutter — `ChoreModel` gained required `createdAt` (fromJson/toJson);
   the optimistic complete/dismiss constructions carry it.
4. Flutter — My Chores pending sorts by `createdAt DESC` with an `id`
   tiebreaker matching the server (scheduler-created recurring instances
   share one transaction timestamp); overdue stays dueDate-ASC pinned; Done
   tab unchanged (completedAt DESC). All Chores list preserves server order.
5. Tests — backend: `test_list_chores_pagination_is_stable` rewritten
   deterministically via a `_backdate_chore` helper (UPDATE created_at in
   the DB; Postgres now() is transaction time so consecutive creates can
   tie and the random-UUID id tiebreaker would be flaky; rowcount asserted
   == 1) + new `test_list_chores_newest_first`. Flutter: widget tests for
   All Chores provider-order rendering (distinct createdAt fixtures) and My
   Chores pending newest-first; E2E asserts the copied (newest) chore
   renders above the original; all fixtures updated.
6. Release: pubspec bumped to 1.0.6+10006; fdroid index rebuilt after
   merge. Deploy-order note (adversarial review): the app's hard-required
   createdAt needs the backend first — both land in the same merge, so no
   skew window in practice.

**Acceptance criteria**: `GET /households/{id}/chores` returns items
newest-created first, each carrying `created_at`; pages disjoint and covering
all rows; All Chores tab (every filter) shows the newest task on top; My
Chores todo shows overdue pinned then pending newest-first; Done tab
unchanged; backend suite 254 @ 97.4% + Flutter 248 green, analyzer clean;
